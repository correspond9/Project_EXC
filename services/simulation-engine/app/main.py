...
Lifespan:
  ...
  4. Start OptionsExpiryWorker (settles expired options every 60 s).
  5. Serve WebSocket endpoint /ws/user/orders for real-time fill push.
"""
import asyncio
import logging
...
from .services.options_expiry import OptionsExpiryWorker
...
async def lifespan(app: FastAPI):
    ...
    options_expiry_worker = OptionsExpiryWorker()
    ...
    options_expiry_task = asyncio.create_task(options_expiry_worker.start(redis_client))
    ...
    monitor_task.cancel()
    options_expiry_task.cancel()
    for task in (mirror_task, subscriber_task, monitor_task, options_expiry_task):
        try:
            await task
